...
export async function generateMetadata({ params }: { params: Promise<{ id: string; slug: string }> }): Promise<Metadata> {
  ...
  if (!post) return { title: "Fact not found" };
  ...
  return {
    ...
    twitter: { card: post.featured_img ? "summary_large_image" : "summary", title: post.title, description: summary(post.content), images: post.featured_img ? [post.featured_img] : undefined },
  };
}
    ...
    <div className="article-layout">
      <article>
        ...
        <header className="article-header">
          ...
          <div className="post-meta"><span>By {post.author_username || "tfacts"}</span><span>{new Date(post.created_at).toLocaleDateString("en", { month: "long", day: "numeric", year: "numeric" })}</span><span><Clock3 size={14} /> {readTime(post.content)} min read</span><span><Eye size={14} /> {post.view_count.toLocaleString()} views</span></div>
        </header>
        <div className={`article-hero art-${post.id % 4}`}>{post.featured_img ? <img src={post.featured_img} alt="" /> : <><span className="art-code">{post.short_code || "TFACT"}</span><div className="art-shape" /></>}</div>
        <BlogPostView html={safeContent} className="article-body editor-rich-content" />
        <CommentSection postId={post.id} initialComments={comments} />
      </article>
        <aside className="article-aside" aria-label="Article actions"><div className="aside-box"><strong>Worth remembering?</strong><p>Save this fact or share it with another curious mind.</p><div className="share-row"><SavePostButton postId={post.id} /><CopyLinkButton url={url} /></div></div></aside>
    </div>
  );
}
