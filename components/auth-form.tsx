...
export function AuthForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${API_URL}/auth/${mode === "login" ? "login" : "register"}/`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(data) });
      ...
      if (mode === "login") {
        if (!auth.access || !auth.refresh || !auth.user_id) {
          throw new Error("The login response did not include a complete session.");
        }
        storeAuthSession(auth.access, auth.refresh, {
          id: auth.user_id,
  ...
  return <form className="form" onSubmit={submit}>
    {mode === "register" && <><div className="field"><label htmlFor="username">Username</label><input id="username" name="username" required autoComplete="username" /></div><div className="field"><label htmlFor="first_name">First name</label><input id="first_name" name="first_name" autoComplete="given-name" /></div><div className="field"><label htmlFor="last_name">Last name</label><input id="last_name" name="last_name" autoComplete="family-name" /></div></>}
    ...
    <p className="auth-switch">{mode === "login" ? <>New to tfacts? <Link href="/register">Create an account</Link></> : <>Already curious with us? <Link href="/login">Sign in</Link></>}</p>
  </form>;
}
